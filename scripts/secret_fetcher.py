...
def get_gcp_secret(secret_name, source):
    logger.info(f"Fetching GCP secret: {secret_name}")
    if not secretmanager:
        logger.error("google-cloud-secret-manager is not installed. Please add it to your requirements.")
        return None
    ...
    try:
        # Note: This requires standard GCP authentication to be set up in the workflow
        # (e.g. via google-github-actions/auth with Workload Identity Federation)
...
def get_aws_secret(secret_name, source):
    ...
    try:
        # Note: This requires standard AWS authentication to be set up in the workflow
        # (e.g. via aws-actions/configure-aws-credentials)
...
def find_secret_source(central_config, name, env, provider_type):
    ...
    for item in central_items:
        if item.get('name') == name and item.get('env') == env and item.get('type') == provider_type:
            return item.get('config', {}).get('source')
    return None
...
def main():
    ...
    for secret_config in secret_list:
        ...
        process_secret(provider, secret_name, target_env, secret_config, github_secrets, github_vars, central_config)

def process_secret(provider, secret_name, target_env, secret_config, github_secrets, github_vars, central_config):
    ...
    # Retrieve the source mapped in central config for this provider, name, and env
    source = find_secret_source(central_config, secret_name, target_env, provider)
    ...
    if provider == 'gh-secret':
        ...
        secret_value = get_gcp_secret(secret_name, source)
    elif provider in ['aws', 'aws-secret-manager']:
        secret_value = get_aws_secret(secret_name, source)
    else:
